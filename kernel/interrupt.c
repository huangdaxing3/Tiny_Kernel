...
//     uint16_t selector; 
//     uint8_t dcount;     //此项为双字计数字段，是门描述符中的第 4 字节, 此项固定值，不用考虑
//     uint8_t attribute; 
...
// struct gate_desc idt[IDT_DESC_CNT];          // idt 是中断描述符表,本质上就是个中断门描述符数组
...
// /* 在中断处理程序数组第 vector_no 个元素中, 注册安装中断处理程序 function */ 
...
/* 完成一般中断处理函数注册及异常名称注册 */
void exception_init(void) {			    // 完成一般中断处理函数注册及异常名称注册
   int i;
   for (i = 0; i < IDT_DESC_CNT; i++) {
